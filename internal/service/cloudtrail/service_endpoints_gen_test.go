...
const (
	packageName = "cloudtrail"
	awsEnvVar   = "AWS_ENDPOINT_URL_CLOUDTRAIL"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cloudtrail"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := cloudtrail.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), cloudtrail.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CloudTrailClient(ctx)
