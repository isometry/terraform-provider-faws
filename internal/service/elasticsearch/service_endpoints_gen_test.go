...
const (
	...
	awsEnvVar   = "AWS_ENDPOINT_URL_ELASTICSEARCH_SERVICE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "elasticsearch_service"

	aliasName0 = "es"
	aliasName1 = "elasticsearchservice"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := elasticsearchservice.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), elasticsearchservice.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ElasticsearchClient(ctx)
