...
const (
	packageName = "datapipeline"
	awsEnvVar   = "AWS_ENDPOINT_URL_DATA_PIPELINE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "data_pipeline"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := datapipeline.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), datapipeline.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.DataPipelineClient(ctx)
