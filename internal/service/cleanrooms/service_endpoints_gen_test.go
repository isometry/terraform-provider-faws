...
const (
	packageName = "cleanrooms"
	awsEnvVar   = "AWS_ENDPOINT_URL_CLEANROOMS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cleanrooms"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := cleanrooms.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), cleanrooms.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CleanRoomsClient(ctx)
