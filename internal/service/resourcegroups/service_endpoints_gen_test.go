...
const (
	packageName = "resourcegroups"
	awsEnvVar   = "AWS_ENDPOINT_URL_RESOURCE_GROUPS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "resource_groups"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := resourcegroups.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), resourcegroups.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ResourceGroupsClient(ctx)
