...
const (
	packageName = "verifiedpermissions"
	awsEnvVar   = "AWS_ENDPOINT_URL_VERIFIEDPERMISSIONS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "verifiedpermissions"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := verifiedpermissions.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), verifiedpermissions.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.VerifiedPermissionsClient(ctx)
