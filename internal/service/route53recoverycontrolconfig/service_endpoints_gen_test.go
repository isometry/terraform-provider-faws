...
const (
	packageName = "route53recoverycontrolconfig"
	awsEnvVar   = "AWS_ENDPOINT_URL_ROUTE53_RECOVERY_CONTROL_CONFIG"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "route53_recovery_control_config"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := route53recoverycontrolconfig.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), route53recoverycontrolconfig.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.Route53RecoveryControlConfigClient(ctx)
