...
const (
	packageName = "devicefarm"
	awsEnvVar   = "AWS_ENDPOINT_URL_DEVICE_FARM"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "device_farm"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := devicefarm.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), devicefarm.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.DeviceFarmClient(ctx)
