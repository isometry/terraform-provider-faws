...
const (
	packageName = "quicksight"
	awsEnvVar   = "AWS_ENDPOINT_URL_QUICKSIGHT"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "quicksight"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := quicksight.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), quicksight.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.QuickSightClient(ctx)
