...
const (
	packageName = "cognitoidentity"
	awsEnvVar   = "AWS_ENDPOINT_URL_COGNITO_IDENTITY"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cognito_identity"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := cognitoidentity.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), cognitoidentity.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CognitoIdentityClient(ctx)
