...
const (
	packageName = "codestarconnections"
	awsEnvVar   = "AWS_ENDPOINT_URL_CODESTAR_CONNECTIONS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "codestar_connections"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := codestarconnections.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), codestarconnections.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CodeStarConnectionsClient(ctx)
