...
const (
	packageName = "apprunner"
	awsEnvVar   = "AWS_ENDPOINT_URL_APPRUNNER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "apprunner"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := apprunner.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), apprunner.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.AppRunnerClient(ctx)
