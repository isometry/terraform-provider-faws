...
const (
	packageName = "customerprofiles"
	awsEnvVar   = "AWS_ENDPOINT_URL_CUSTOMER_PROFILES"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "customer_profiles"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := customerprofiles.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), customerprofiles.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CustomerProfilesClient(ctx)
