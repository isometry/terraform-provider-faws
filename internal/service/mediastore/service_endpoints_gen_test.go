...
const (
	packageName = "mediastore"
	awsEnvVar   = "AWS_ENDPOINT_URL_MEDIASTORE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "mediastore"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := mediastore.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), mediastore.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.MediaStoreClient(ctx)
