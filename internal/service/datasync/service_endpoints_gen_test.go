...
const (
	packageName = "datasync"
	awsEnvVar   = "AWS_ENDPOINT_URL_DATASYNC"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "datasync"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := datasync.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), datasync.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.DataSyncClient(ctx)
