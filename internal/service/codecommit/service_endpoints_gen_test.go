...
const (
	packageName = "codecommit"
	awsEnvVar   = "AWS_ENDPOINT_URL_CODECOMMIT"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "codecommit"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := codecommit.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), codecommit.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CodeCommitClient(ctx)
