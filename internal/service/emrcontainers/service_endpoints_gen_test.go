...
const (
	packageName = "emrcontainers"
	awsEnvVar   = "AWS_ENDPOINT_URL_EMR_CONTAINERS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "emr_containers"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := emrcontainers.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), emrcontainers.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.EMRContainersClient(ctx)
