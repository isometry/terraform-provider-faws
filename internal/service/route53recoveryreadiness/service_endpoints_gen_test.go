...
const (
	packageName = "route53recoveryreadiness"
	awsEnvVar   = "AWS_ENDPOINT_URL_ROUTE53_RECOVERY_READINESS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "route53_recovery_readiness"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := route53recoveryreadiness.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), route53recoveryreadiness.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.Route53RecoveryReadinessClient(ctx)
