...
const (
	packageName = "elastictranscoder"
	awsEnvVar   = "AWS_ENDPOINT_URL_ELASTIC_TRANSCODER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "elastic_transcoder"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := elastictranscoder.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), elastictranscoder.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ElasticTranscoderClient(ctx)
