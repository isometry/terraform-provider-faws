...
const (
	packageName = "invoicing"
	awsEnvVar   = "AWS_ENDPOINT_URL_INVOICING"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "invoicing"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := invoicing.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), invoicing.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.InvoicingClient(ctx)

	var result apiCallParams

	input := invoicing.ListInvoiceUnitsInput{}
	_, err := client.ListInvoiceUnits(ctx, &input,
		func(opts *invoicing.Options) {
