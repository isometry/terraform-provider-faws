...
const (
	packageName = "codegurureviewer"
	awsEnvVar   = "AWS_ENDPOINT_URL_CODEGURU_REVIEWER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "codeguru_reviewer"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := codegurureviewer.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), codegurureviewer.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CodeGuruReviewerClient(ctx)
