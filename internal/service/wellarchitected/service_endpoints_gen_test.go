...
const (
	packageName = "wellarchitected"
	awsEnvVar   = "AWS_ENDPOINT_URL_WELLARCHITECTED"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "wellarchitected"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := wellarchitected.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), wellarchitected.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.WellArchitectedClient(ctx)
