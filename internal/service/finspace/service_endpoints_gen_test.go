...
const (
	packageName = "finspace"
	awsEnvVar   = "AWS_ENDPOINT_URL_FINSPACE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "finspace"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := finspace.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), finspace.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.FinSpaceClient(ctx)
