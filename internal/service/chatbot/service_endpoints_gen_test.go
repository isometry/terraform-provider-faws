...
const (
	packageName = "chatbot"
	awsEnvVar   = "AWS_ENDPOINT_URL_CHATBOT"
...
func TestEndpointConfiguration(t *testing.T) { //nolint:paralleltest // uses t.Setenv
	...
	// Chatbot uses a regional endpoint but is only available in one region or a limited number of regions.
	// The provider overrides the region for Chatbot, but the AWS SDK's endpoint resolution returns one for the current region.
...
func defaultEndpoint(region string) (url.URL, error) {
	r := chatbot.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), chatbot.EndpointParameters{
