...
const (
	packageName = "cloudwatch"
	awsEnvVar   = "AWS_ENDPOINT_URL_CLOUDWATCH"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cloudwatch"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := cloudwatch.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), cloudwatch.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CloudWatchClient(ctx)
