...
const (
	packageName = "servicequotas"
	awsEnvVar   = "AWS_ENDPOINT_URL_SERVICE_QUOTAS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "service_quotas"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := servicequotas.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), servicequotas.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ServiceQuotasClient(ctx)
