...
const (
	packageName = "elasticbeanstalk"
	awsEnvVar   = "AWS_ENDPOINT_URL_ELASTIC_BEANSTALK"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "elastic_beanstalk"

	aliasName0 = "beanstalk"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := elasticbeanstalk.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), elasticbeanstalk.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ElasticBeanstalkClient(ctx)
