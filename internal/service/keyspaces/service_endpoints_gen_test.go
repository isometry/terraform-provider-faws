...
const (
	packageName = "keyspaces"
	awsEnvVar   = "AWS_ENDPOINT_URL_KEYSPACES"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "keyspaces"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := keyspaces.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), keyspaces.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.KeyspacesClient(ctx)
