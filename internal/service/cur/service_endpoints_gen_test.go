...
const (
	packageName = "cur"
	awsEnvVar   = "AWS_ENDPOINT_URL_COST_AND_USAGE_REPORT_SERVICE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cost_and_usage_report_service"

	aliasName0 = "costandusagereportservice"
)
...
func TestEndpointConfiguration(t *testing.T) { //nolint:paralleltest // uses t.Setenv
	...
	// Cost and Usage Report uses a regional endpoint but is only available in one region or a limited number of regions.
	// The provider overrides the region for Cost and Usage Report, but the AWS SDK's endpoint resolution returns one for the current region.
...
func defaultEndpoint(region string) (url.URL, error) {
	r := costandusagereportservice.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), costandusagereportservice.EndpointParameters{
