...
const (
	packageName = "comprehend"
	awsEnvVar   = "AWS_ENDPOINT_URL_COMPREHEND"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "comprehend"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := comprehend.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), comprehend.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ComprehendClient(ctx)
