...
const (
	packageName = "opensearchserverless"
	awsEnvVar   = "AWS_ENDPOINT_URL_OPENSEARCHSERVERLESS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "opensearchserverless"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := opensearchserverless.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), opensearchserverless.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.OpenSearchServerlessClient(ctx)
