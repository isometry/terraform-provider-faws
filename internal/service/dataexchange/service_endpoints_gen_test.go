...
const (
	packageName = "dataexchange"
	awsEnvVar   = "AWS_ENDPOINT_URL_DATAEXCHANGE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "dataexchange"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := dataexchange.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), dataexchange.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.DataExchangeClient(ctx)
