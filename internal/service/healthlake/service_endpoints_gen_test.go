...
const (
	packageName = "healthlake"
	awsEnvVar   = "AWS_ENDPOINT_URL_HEALTHLAKE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "healthlake"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := healthlake.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), healthlake.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.HealthLakeClient(ctx)
