...
const (
	packageName = "lightsail"
	awsEnvVar   = "AWS_ENDPOINT_URL_LIGHTSAIL"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "lightsail"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := lightsail.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), lightsail.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.LightsailClient(ctx)
