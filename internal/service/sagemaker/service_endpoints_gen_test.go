...
const (
	packageName = "sagemaker"
	awsEnvVar   = "AWS_ENDPOINT_URL_SAGEMAKER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "sagemaker"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := sagemaker.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), sagemaker.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.SageMakerClient(ctx)
