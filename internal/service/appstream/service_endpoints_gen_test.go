...
const (
	packageName = "appstream"
	awsEnvVar   = "AWS_ENDPOINT_URL_APPSTREAM"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "appstream"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := appstream.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), appstream.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.AppStreamClient(ctx)
