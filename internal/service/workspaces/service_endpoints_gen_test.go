...
const (
	...
	awsEnvVar   = "AWS_ENDPOINT_URL_WORKSPACES"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "workspaces"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := workspaces.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), workspaces.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.WorkSpacesClient(ctx)
