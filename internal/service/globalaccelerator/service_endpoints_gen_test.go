...
const (
	packageName = "globalaccelerator"
	awsEnvVar   = "AWS_ENDPOINT_URL_GLOBAL_ACCELERATOR"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "global_accelerator"
)
...
func TestEndpointConfiguration(t *testing.T) { //nolint:paralleltest // uses t.Setenv
	...
	// Global Accelerator uses a regional endpoint but is only available in one region or a limited number of regions.
	// The provider overrides the region for Global Accelerator, but the AWS SDK's endpoint resolution returns one for the current region.
...
func defaultEndpoint(region string) (url.URL, error) {
	r := globalaccelerator.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), globalaccelerator.EndpointParameters{
