...
const (
	packageName = "wafregional"
	awsEnvVar   = "AWS_ENDPOINT_URL_WAF_REGIONAL"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "waf_regional"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := wafregional.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), wafregional.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.WAFRegionalClient(ctx)
