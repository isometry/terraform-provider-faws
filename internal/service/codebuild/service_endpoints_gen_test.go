...
const (
	packageName = "codebuild"
	awsEnvVar   = "AWS_ENDPOINT_URL_CODEBUILD"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "codebuild"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := codebuild.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), codebuild.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CodeBuildClient(ctx)
