...
const (
	packageName = "memorydb"
	awsEnvVar   = "AWS_ENDPOINT_URL_MEMORYDB"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "memorydb"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := memorydb.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), memorydb.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.MemoryDBClient(ctx)
