...
const (
	packageName = "codeguruprofiler"
	awsEnvVar   = "AWS_ENDPOINT_URL_CODEGURUPROFILER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "codeguruprofiler"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := codeguruprofiler.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), codeguruprofiler.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CodeGuruProfilerClient(ctx)
