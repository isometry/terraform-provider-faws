...
const (
	packageName = "ssmcontacts"
	awsEnvVar   = "AWS_ENDPOINT_URL_SSM_CONTACTS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "ssm_contacts"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := ssmcontacts.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), ssmcontacts.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.SSMContactsClient(ctx)
