...
const (
	packageName = "appintegrations"
	awsEnvVar   = "AWS_ENDPOINT_URL_APPINTEGRATIONS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "appintegrations"

	aliasName0 = "appintegrationsservice"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := appintegrations.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), appintegrations.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.AppIntegrationsClient(ctx)
