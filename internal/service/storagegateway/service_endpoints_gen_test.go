...
const (
	packageName = "storagegateway"
	awsEnvVar   = "AWS_ENDPOINT_URL_STORAGE_GATEWAY"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "storage_gateway"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := storagegateway.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), storagegateway.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.StorageGatewayClient(ctx)
