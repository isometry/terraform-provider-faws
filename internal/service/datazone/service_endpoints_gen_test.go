...
const (
	packageName = "datazone"
	awsEnvVar   = "AWS_ENDPOINT_URL_DATAZONE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "datazone"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := datazone.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), datazone.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.DataZoneClient(ctx)
