...
const (
	packageName = "ssmquicksetup"
	awsEnvVar   = "AWS_ENDPOINT_URL_SSM_QUICKSETUP"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "ssm_quicksetup"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := ssmquicksetup.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), ssmquicksetup.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.SSMQuickSetupClient(ctx)
