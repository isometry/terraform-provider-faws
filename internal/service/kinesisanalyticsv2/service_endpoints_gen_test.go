...
const (
	packageName = "kinesisanalyticsv2"
	awsEnvVar   = "AWS_ENDPOINT_URL_KINESIS_ANALYTICS_V2"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "kinesis_analytics_v2"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := kinesisanalyticsv2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), kinesisanalyticsv2.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.KinesisAnalyticsV2Client(ctx)
